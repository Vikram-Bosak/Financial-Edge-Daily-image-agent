# Financial Edge Daily Image Agent

An automated, cloud-based Python agent that monitors Google News and Twitter (X) for the latest trends in finance, stock markets, and economy, uses NVIDIA Nemotron LLM to generate highly engaging caption copy, designs custom posters using Pillow with a professional finance theme, uploads them automatically to your Facebook Page, and alerts you via a Discord Channel Webhook.

## Features
- **24/7 Automation**: Runs every 2 hours (12 posts per day) via GitHub Actions scheduler.
- **Smart Scraper**: Scrapes Google News RSS feeds for financial terms and contains stub integration for Twitter (X) search endpoints.
...
- **Dynamic Poster Designer**: Custom Pillow engine that stitches background photos, overlays vertical gradients, adds a circular badge, centers typography, and outputs high-definition 1080x1350 images in a premium finance theme (deep green / charcoal).
