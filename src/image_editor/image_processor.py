...
def draw_circular_badge(base_img, flag_url_or_path, size=300, pos_x=70, pos_y=600):
    ...
    if not badge_img:
        badge_img = Image.new('RGB', (size, size), "#0B5C43")
...
def render_multicolor_text_centered(draw, text, y_pos, font, max_width, img_width, dry_run=False):
    ...
    for line_tokens in lines:
        ...
        for token in line_tokens:
            ...
            if token["highlight"]:
                # Premium finance-aligned highlights (Emerald Green, Gold/Yellow, Bright Cyan)
                HIGHLIGHT_COLORS = ["#2ECC71", "#FFD700", "#00E5FF", "#58D68D"]
...
def create_facebook_post(image_url, image_url_2, headline, source_name="NEWS", output_path="output.jpg", logo_path="assets/logo/logo.png", hook_text="", circle_image_url=None):
    ...
    if img1 and img2:
        ...
        # Professional dark financial background (emerald green/charcoal gradient)
        draw_temp = ImageDraw.Draw(base_img)
        draw_temp.rectangle([(0, 0), (base_width, base_height)], fill="#0B2F26")
        
    overlay = Image.new('RGBA', (base_width, base_height), (0,0,0,0))
    draw_gradient(overlay, 600, 1050, color_start=(7,15,14,0), color_end=(7,15,14,255))
    base_img = Image.alpha_composite(base_img.convert('RGBA'), overlay).convert('RGB')
    
    draw = ImageDraw.Draw(base_img)
    draw.rectangle([(0, 1050), (base_width, base_height)], fill="#070F0E")
    ...
    if os.path.exists(banner_path):
        ...
        try:
            ...
            brand_text = os.getenv("BRANDING_TEXT", "FINANCIAL EDGE DAILY").upper()
            bbox = draw.textbbox((0, 0), brand_text, font=brand_font)
            brand_w = bbox[2] - bbox[0]
            bx = (base_width - brand_w) // 2
            by = text_start_y - 65
            draw.text((bx, by), brand_text, font=brand_font, fill="#2ECC71")
        except Exception as e:
